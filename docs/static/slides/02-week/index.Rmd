...
> Regression analysis is a statistical methodology that utilizies the relation between two or more quantitative variables so that a response variable can be predictted from the others
...
> Sales of a product predicted by amount of advertising spent
...
Notice how the relation is not perfect
]
...
# Sneak peak: More then one predictor variable
...
.pull-right[
There is a probability distributioon of Y for each level of $X_i$
]
...
This does not imply that an increase inn covabulary causes a faster writing speed

What is more likely is that a 3rd variable such as "age of child" positively affects both
...
> Relationship bettween he number of bids requested for contractors during a week and the time required to prepare the bids
...
If we suppose that the $i$th week, $X_i = 45$ then we would expect the number of hours spend preparing to be 104. But if the actual number of hours $Y_i = 108$ then the error is $\varepsilon_i = 4$
...
$\beta_0$: $Y$ intercept of regression line. When $X = 0$ gives mean of probability distribution of $Y$.
...
we can re arrange the terms in $(2)$ that
...
1. **Variance** it can be shown that $b_0, b_1$ gives the minimum variance in the group of linnear and unbiased estimators.

These two points are actually part of the the Gauss-Markov theorem
...
The $i$th residual is denoetd by $e_i$ and is defined as
